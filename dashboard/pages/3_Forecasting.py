...
This module renders the forecasting dashboard for the agricultural commodities pipeline.
It provides ensemble forecasts, model comparisons, and historical percentile context.
Data is dynamically loaded from the central DuckDB database to ensure up-to-date insights.
"""
...
def load_data() -> pd.DataFrame:
    """
    Load historical prices from DuckDB and pivot to a wide format 
    ...
    # Pivot so that dates are the index and commodities are columns
...
def load_signals() -> pd.DataFrame:
    """
    Load the most recent signal metrics generated by the pipeline from DuckDB.
    """
    query = "SELECT * FROM signals WHERE run_date = (SELECT MAX(run_date) FROM signals)"
...
def load_forecasts() -> dict:
    """
    Load the latest forecasts from DuckDB and parse the JSON payloads 
    to reconstruct the dictionary schema expected by the frontend.
    """
    query = "SELECT * FROM forecasts WHERE run_date = (SELECT MAX(run_date) FROM forecasts)"
    with db.get_connection() as conn:
        df = conn.execute(query).df()
    ...
    for _, row in df.iterrows():
        ...
        if comm not in forecasts_dict["forecasts"]:
            forecasts_dict["forecasts"][comm] = {
                "current_price": last_prices.get(comm, 0.0),
                "individual_models": {}
            }
        
        # Parse DuckDB JSON strings
        ...
        if model_name == 'ensemble':
            # Map numeric DB confidence column back to categorical values if necessary
            conf_val = row.get('confidence', 0)
            if isinstance(conf_val, (int, float)):
                if conf_val >= 0.8: conf_str = 'high'
                elif conf_val >= 0.5: conf_str = 'medium'
                else: conf_str = 'low'
            else:
                conf_str = str(conf_val)

            forecasts_dict["forecasts"][comm]["ensemble"] = {
                "predictions": preds,
                "confidence": conf_str,
                "avg_mape": metrics.get('mape', 0) if metrics else 0
            }
        else:
            # Build individual model metadata
            model_data = {
                "predictions": preds,
                "metrics": metrics or {},
                "method": model_name
            ...
            # Map out volatility forecast for GARCH bounds explicitly
            ...
            forecasts_dict["forecasts"][comm]["individual_models"][model_name] = model_data

    return forecasts_dict

# Extract standard datasets
df = load_data()
forecasts_data = load_forecasts()
signals_df = load_signals()

if df.empty:
    st.error("No historical data found in the database. Ensure the pipeline has run.")
    st.stop()

if not forecasts_data:
    st.warning("No forecast data available in the database. Ensure models have executed.")
    st.stop()
...
if not available:
    st.warning("No successful forecasts found in the latest database run.")
    st.stop()
...
# ------------------------------------------------------------------ #
# Period Breakdown & Model Performance                               #
# ------------------------------------------------------------------ #
...
st.markdown('<p class="section-title">Historical Context — Forecast Position</p>', unsafe_allow_html=True)
...
**Model pool (frequency and sample-size adaptive):**

Statistical core (all frequencies): ARIMA · SARIMA · Exponential Smoothing

Volatility (all frequencies, >= 30 observations): GARCH(1,1) — used for confidence bands only, excluded from ensemble price predictions.

Regularised linear (>= 60 observations): Ridge Regression

Supervised ML (daily/weekly only, >= 150 observations): Gradient Boosting · Random Forest · XGBoost · LASSO

**Rationale:** Supervised ML models require a training set materially larger than their feature space (~13 lag and calendar features). With monthly or quarterly data, including them would introduce overfitting that contaminates ensemble weights. The pool expands automatically when the series length justifies it.

**Validation:** Hold-out test set (size adapted to data frequency). Models are ranked by MASE (preferred) or MAPE. Ensemble = exponential-weighted average of the top 3 price-forecast models.

**GARCH bands:** Log-return volatility forecasts converted back to price space. 1.645σ = 90% confidence interval assuming normally distributed returns.

**Confidence levels:** HIGH = MASE < 0.8 · MEDIUM = MASE < 1.2 · LOW = MASE >= 1.2

**Disclaimer:** Forecasts are probabilistic scenarios based on historical patterns. External shocks (geopolitical events, weather, policy changes) are not modelled.
